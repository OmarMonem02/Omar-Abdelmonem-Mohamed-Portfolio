...
const Button = ({ children, className, ...props }) => (
  <button {...props} className={"px-5 py-2 rounded-2xl border border-neutral-700 hover:bg-neutral-800 transition " + className}>
    {children}
  </button>
);
...
export default function OmarPortfolio() {
  ...
  return (
    <div
      ...
      className={(dark ? "bg-black text-white" : "bg-white text-black") + " min-h-screen relative overflow-hidden transition-colors duration-500"}
    >
      {/* LOADING INTRO */}
      <AnimatePresence>
        {loading && (
          <motion.div
            ...
            className={(dark ? "bg-black" : "bg-white") + " fixed inset-0 flex items-center justify-center z-50"}
          >
            <motion.h1
            ...
            >
              Omar Abdelmonem<br/>Software Engineer
            </motion.h1>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="px-6 md:px-16 py-10 space-y-36 relative z-10">
        {/* HERO */}
        <motion.section style={{ y: yHero }} className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-12 items-center min-h-[80vh] text-center md:text-left">
          <motion.div
            initial={{ opacity: 0, y: 80 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.9 }}
            className="space-y-6 text-center md:text-left items-center md:items-start"
          >
            ...
            <div className="flex flex-wrap gap-4">
              <Button onClick={() => projectsRef.current?.scrollIntoView({ behavior: "smooth" })}>View Projects</Button>
              <Button onClick={() => contactMeRef.current?.scrollIntoView({ behavior: "smooth" })} className="bg-gradient-to-br from-purple-500 to-blue-500 text-white border-transparent hover:from-purple-600 hover:to-blue-600">Contact Me</Button>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            transition={{ duration: 0.9 }}
            className="flex justify-center relative order-1 md:order-2 mb-6 md:mb-0"
          >
            <motion.div
            ...
            >
              ...
              <div className="pointer-events-none absolute inset-0 rounded-3xl opacity-0 hover:opacity-100 transition duration-500 bg-[radial-gradient(circle_at_center,rgba(168,85,247,0.25),transparent_70%)]" />
            </motion.div>
            ...
            <div className="hidden md:block">
              {techStack.map((tech, i) => (
                <motion.div
                  ...
                  className="absolute text-xs bg-neutral-900 border border-neutral-700 px-3 py-1 rounded-full"
                  style={{ top: 30 + i * 40, left: i % 2 === 0 ? 10 : 220 }}
                >
                  {tech}
                </motion.div>
              ))}
        ...
        >
          ...
          <p className="text-neutral-400 leading-relaxed">
            ...
            strong experience integrating REST APIs, payment systems,
            role-based access control, and real-time data handling.
          </p>
        </motion.section>
        ...
        <section ref={contactMeRef}>
          ...
          <motion.section
            ...
            transition={{ duration: 0.6 }}
            viewport={{ once: true }}
            className="space-y-6 order-2 md:order-1 items-center md:items-start"
          >
            <h2 className="text-3xl font-semibold">Contact</h2>
            <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 text-neutral-400">
              <div className="flex items-center gap-2"><Mail size={18} /> omarabdelmonem91@gmail.com</div>
              <div className="flex items-center gap-2"><Phone size={18} /> +201093818755</div>
              <div className="flex items-center gap-2"><MapPin size={18} /> El-Sheikh Zayed, Giza</div>
              <div className="flex gap-6 pt-4">
                <a href="https://github.com/OmarMonem02" target="_blank"><Github /></a>
                <a href="https://linkedin.com/in/omar-monem2002" target="_blank"><Linkedin /></a>
              </div>
            </div>
          </motion.section>
        </section>
